...
function setLoginName(name: string): void {
...
function setPasscode(code: number): void {
    var inputBox: string = $('#passcodeBox').val();
...
function removeLastCharPasscode(): void {
    var inputBox: string = $('#passcodeBox').val();
...
function submit() {
    var id: string = $('#selectedLogin').html();
    var pass: string = $('#passcodeBox').val();

    //Simple validation for fields passcode and login
    var idValidation: boolean = false;
    var passValidation: boolean = false;
...
function addOrUpdateOrder(name: string, price: number, id: number) {
...
function addOrder(name: string, price: number, id: number) {
...
function updateOrder(id: number, price: number) {
    var quantity: number = parseFloat($('#orderQuantity_' + id)[0].innerHTML);
    quantity++;
...
function deleteOrder(id: number, price: number) {
    var quantity: number = parseFloat($('#orderQuantity_' + id)[0].innerHTML);
    if (quantity > 1) {
        quantity--;
        $('#orderQuantity_' + id)[0].innerHTML = (quantity).toString();
...
function updateTotal(price: number, isAddition: boolean) {
    var currentPrice: number = parseFloat($('#totalPrice')[0].innerHTML);
    var currentQuantity: number = parseInt($('#totalQuantity')[0].innerHTML);
